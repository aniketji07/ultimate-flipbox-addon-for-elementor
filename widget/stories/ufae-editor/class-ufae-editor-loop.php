...
if ( ! class_exists( 'Ufae_Editor_Loop' ) ) {
	...
	class Ufae_Editor_Loop {
		...
		private function render_title( $side ) {
			...
			let title = item['ufae_' + side + '_title'] && '' !== item['ufae_' + side + '_title'] ? item['ufae_' + side + '_title'] : false;

			if (title) {
				let title_tag = settings.ufae_title_tag && '' !== settings.ufae_title_tag ? settings.ufae_title_tag : 'h3';
